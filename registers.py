...
class Control2(IntEnum):
    ...
    """
    RESET = 0x01  # 0: Normal operation. 1: software based time adjustment (see datasheet).
    HOUR_MODE = 0x02 # 0: 24 hour mode (default). 1: 12 hour mode.
    EVENT_INT_ENABLE = 0x04  # Enables the event interrupt on the INT pin (disabled by default).
    ALARM_INT_ENABLE = 0x08  # Enables the alarm interrupt on the INT pin (disabled by default).
    TIMER_INT_ENABLE = 0x10  # Enables the timer interrupt on the INT pin (disabled by default).
    UPDATE_INT_ENABLE = 0x20  # Enables the update interrupt on the INT pin (disabled by default).
    CLOCK_OUTPUT_INT_ENABLE = 0x40  # Enables the clock output interrupt on the INT pin (disabled by default).
