...
def test_set_time():
    ...
    assert i2c.registers[rtc.SECONDS] == ((56 // 10) << 4) | (56 % 10)
    assert i2c.registers[rtc.MINUTES] == ((34 // 10) << 4) | (34 % 10)
    assert i2c.registers[rtc.HOURS] == ((12 // 10) << 4) | (12 % 10)

def test_get_time():
    i2c = MockI2C()
    rtc = RV3028(i2c)

    i2c.registers[rtc.SECONDS] = ((56 // 10) << 4) | (56 % 10)
    i2c.registers[rtc.MINUTES] = ((34 // 10) << 4) | (34 % 10)
    i2c.registers[rtc.HOURS] = ((12 // 10) << 4) | (12 % 10)
...
def test_set_date():
    ...
    assert i2c.registers[rtc.YEAR] == ((21 // 10) << 4) | (21 % 10)
    assert i2c.registers[rtc.MONTH] == ((9 // 10) << 4) | (9 % 10)
    assert i2c.registers[rtc.DATE] == ((30 // 10) << 4) | (30 % 10)
    assert i2c.registers[rtc.WEEKDAY] == ((4 // 10) << 4) | (4 % 10)
...
def test_get_date():
    ...
    i2c.registers[rtc.YEAR] = ((21 // 10) << 4) | (21 % 10)
    i2c.registers[rtc.MONTH] = ((9 // 10) << 4) | (9 % 10)
    i2c.registers[rtc.DATE] = ((30 // 10) << 4) | (30 % 10)
    i2c.registers[rtc.WEEKDAY] = ((4 // 10) << 4) | (4 % 10)
...
def test_set_alarm():
    ...
    assert i2c.registers[rtc.ALARM_MINUTES] == ((45 // 10) << 4) | (45 % 10)
    assert i2c.registers[rtc.ALARM_HOURS] == ((6 // 10) << 4) | (6 % 10)
    assert i2c.registers[rtc.ALARM_WEEKDAY] == ((1 // 10) << 4) | (1 % 10)
...
def test_check_alarm():
    ...
    # Set the alarm flag
    i2c.registers[rtc.STATUS] |= 0x04

    result = rtc.check_alarm()
    assert result == True
    # Alarm flag should be cleared after check
    assert (i2c.registers[rtc.STATUS] & 0x04) == 0
...
def test_enable_trickle_charger_valid():
    ...
    backup_reg = i2c.registers[rtc.EEPROM_BACKUP]
...
def test_configure_evi_enable():
    ...
    event_control = i2c.registers[rtc.EVENT_CONTROL]
    assert event_control == 0x40  # EHL = 1, ET = 00

    control2 = i2c.registers[rtc.CONTROL2]
...
def test_get_event_timestamp():
    ...
    i2c.registers[rtc.TIMESTAMP_COUNT] = 1
    i2c.registers[rtc.TIMESTAMP_SECONDS] = ((50 // 10) << 4) | (50 % 10)
    i2c.registers[rtc.TIMESTAMP_MINUTES] = ((40 // 10) << 4) | (40 % 10)
    i2c.registers[rtc.TIMESTAMP_HOURS] = ((14 // 10) << 4) | (14 % 10)
    i2c.registers[rtc.TIMESTAMP_DATE] = ((15 // 10) << 4) | (15 % 10)
    i2c.registers[rtc.TIMESTAMP_MONTH] = ((8 // 10) << 4) | (8 % 10)
    i2c.registers[rtc.TIMESTAMP_YEAR] = ((22 // 10) << 4) | (22 % 10)
...
def test_clear_event_flag():
    ...
    # Set the event flag
    i2c.registers[rtc.STATUS] |= 0x02

    rtc.clear_event_flag()
    assert (i2c.registers[rtc.STATUS] & 0x02) == 0  # EVF bit is cleared
...
def test_clear_backup_switchover_flag():
    ...
    assert (i2c.registers[rtc.STATUS] & 0x20) == 0  # BSF bit is cleared